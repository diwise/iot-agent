...
func TestElsysDigital1True(t *testing.T) {
	is, _ := testSetup(t)
	ue, err := facades.New("servanet")([]byte(`{"data": "DQEaAA==", "fPort": 5, "devEui": "abc123", "object": {"digital": 1, "digital2": 0},  "timestamp": "2024-08-05T11:23:45.347949876Z", "deviceName": "abc123", "sensorType": "Elsys"}`))
	is.NoErr(err)
...
func TestElsysDigital1True_lwm2m(t *testing.T) {
	is, _ := testSetup(t)
	ue, err := facades.New("servanet")([]byte(`
	{
		"data": "DQEaAA==",
		"fPort": 5,
		"devEui": "abc123",
...
func TestElsysDigital1False(t *testing.T) {
	is, _ := testSetup(t)
	ue, err := facades.New("servanet")([]byte(`{"data": "DQAaAA==", "fPort": 5, "devEui": "abc123", "object": {"digital": 1, "digital2": 0},  "timestamp": "2024-08-05T11:23:45.347949876Z", "deviceName": "abc123", "sensorType": "Elsys"}`))
	is.NoErr(err)
	p, err := decodePayload(ue.Data)
	is.NoErr(err)
...
func TestElsysCO2Decoder(t *testing.T) {
	...
	ue, _ := facades.New("servanet")([]byte(elsysCO2))
	payload, err := Decoder(context.Background(), ue)
	is.NoErr(err)
	objects, err := Converter(context.Background(), "abc123", payload, ue.Timestamp)
	is.NoErr(err)
...
func TestElsysTemperatureDecoder(t *testing.T) {
	...
	ue, _ := facades.New("servanet")([]byte(elsysTemp))
	payload, err := Decoder(context.Background(), ue)
	is.NoErr(err)
	objects, err := Converter(context.Background(), "abc123", payload, ue.Timestamp)	
	is.NoErr(err)
...
func TestElsysPumpbrunnarDecoder(t *testing.T) {
	...
	ue, err := facades.New("netmore")([]byte(elt2hp))
	is.NoErr(err)
	payload, err := Decoder(context.Background(), ue)
	is.NoErr(err)
	objects, err := Converter(context.Background(), "abc123", payload, ue.Timestamp)
...
func TestElsysElt2hpTrue(t *testing.T) {
	...
	ue, _ := facades.New("netmore")([]byte(elt2hp_true))
	payload, err := Decoder(context.Background(), ue)
	is.NoErr(err)
	objects, err := Converter(context.Background(), "abc123", payload, ue.Timestamp)
	is.NoErr(err)
